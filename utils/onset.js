...
const MIN_SEGMENT_LENGTH = 0.100; // minimum duration of one segment in seconds

// Get onsets based on changes in spectrum flux
function getOnsets(samples, min_length = MIN_SEGMENT_LENGTH){
  ...
  var fft  = new FFT(SAMPLE_SIZE, sampleRate);
  var fft2 = new FFT(FFT2SIZE,    sampleRate / SAMPLE_SIZE);
  ...
  for (let i = 1; i < peaks_.length - 1; i++) {
    if (peaks_[i] > 0 && peaks_[i-1] == 0) { // avoid onset right after another onset
      var onset = i * SAMPLE_SIZE / sampleRate; // in seconds
      if (onset - prevOnset > MIN_SEGMENT_LENGTH){
        peaks.push(onset);
        prevOnset = onset;
      }
    }
